...
Uma operadora de plano de saúde que atende em quatro regiões do Brasil percebeu que os gastos do seguro saúde aumentaram de forma considerável e seus diretores precisam monitorar a evolução dos gastos.

Esses diretores fizeram diversas perguntas e gostariam de poder visualizar as respostas em uma única tela ou visualização.

Os dados disponíveis correspondem ao ano anterior dos usuários da operadora e possuem as seguintes colunas: **Idade**, **Sexo**, **IMC (Índice de Massa Corpórea)**, se **é riança**, se **é fumante**, a **Região** do usuário e o **Valor de Seguro Saúde** de cada cliente.
...
Os dados fornecidos contém "problemas", erros que foram propositalmente inseridos no dataset, simulando divergências do mundo real.
A proposta é, além de criar os gráficos e atender as solicitações dos diretores, detectar os problemas nos dados e executar um Data Cleansing, interpretar corretamente o estudo de caso e decidir como resolver os equívocos.
...
No primeiro momento, é possível logo identificar que os nomes das colunas não estão de acordo com as informações, assim como as duas primeiras linhas estão repetidas e não representam dados válidos.
...
Em seguida, a opção _Remover Linhas Principais_ foi selecionada para excluir o dado que antes estava repetido.
...
Valores numéricos onde não deveriam estar e em branco foram encontrados.
...
Assim, as duas novas colunas chamadas **Faixa Etária** e **Meia Idade** foram preenchidas com informações que separavam os clientes da seguradora em grupos por faixa etária e dois grupos de maior ou menor de 50 anos.
...
Foi novamente usado o ***Cartão*** onde foi utilizada a medida **Média Idade** para apresentar o resultado.
...
Analisando o ***Gráfico de barras clusterizado na vertical***, percebe-se que os clientes da faixa etária dos 50 aos 59 anos são os que mais gastam com o seguro saúde em praticamente todas as regiões, salvo o Sudeste, que são as pessoas entre 18 a 29 anos.
...
Outro ponto crucial é que, ao utilizar um filtro e colocar a coluna **Idade** em ordem crescente, vê-se que todas as pessoas são maiores de 18 anos, salientando ainda mais o fato de que a coluna **Criança** não representa se elas são ou não crianças.
...
Sem esses dados, fica impraticável trazer uma solução confiável e íntegra a área de negócios.
...
Com a adição de três filtros distintos, um para **Sexo**, um para **Região** e outro para **Meia Idade**, os gráficos interagem entre si e mostram, de maneira específica, qual a média de dinheiro gasto por homens da região Sudeste e que estão acima dos 50 anos de idade no já demonstrado ***Gráfico de barras clusterizado na horizontal***.
O grupo de dados **Meia Idade** foi usado com o intuito de auxiliar essa segmentação.

Para que a comparação seja feita de uma melhor maneira, uma ***Tabela*** foi adicionada com as informações de média de valor do seguro saúde por região e ela **não interage com os filtros**, isto é, ela não sofre alterações quando a filtragem ocorre e, por isso, seus valores permanecem estáticoos.

Assim, é possível visualizar que os homens com mais de 50 anos e da região Sudeste, comparados com a média de gastos da região, gastam bem mais do seguro saúde.

---

#### Extra. Qual a média de valor gasta por faixa etária e se a pessoa é fumante ou não?
...
A primeira modifição na proposta do caso foi na adição de uma quarta ***Segmentação de Dados*** para filtrar todas as pessoas que são fumantes ou não, a fim de trazer mais especificidade nas consultas dos diretores.

Depois disso, adotei um ***Tornado Chart*** (que foi baixado pela Store da Microsoft) para responder a pergunta proposta.
Nele, o grupo de dados **Faixa Etária** foi colocada no campo Grupo, enquanto na Legenda foram colocados os dados da coluna **Fumante**. Por fim, o cálculo da média do **Valor Seguro Saúde** foi adicionado no campo Valores.

Com essas configurações, tornou-se possível visualizar a média de valor gastar por faixa etária e se a pessoa é fumante ou não.

## Conclusão

O estudo de caso trouxe uma valor inestimável para o meu aprendizado com Power BI.
Desde às técnicas de Data Cleansing até a confecção dos gráficos, esses recursos trouxeram insights inimagináveis que, com certeza, serão sempre utilizados em meus próximos projetos.
...
O projeto também me fez estudar mais a fundo quais cores e quais gráficos funcionam melhor para quais situações, melhorando as técnicas storytelling com dados.

Senti que necessito aprimorar conceitos mais avançados DAX e M, assim como a confecção das visualizações.
